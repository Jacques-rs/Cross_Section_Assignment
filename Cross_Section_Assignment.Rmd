...
pacman::p_load(tidyverse, dplyr, lubridate, AER, broom, plm, corrplot, psych)
...
```{r}
read_csv(file = "./data/owid-covid-data.csv",
         show_col_types = F) %>% select(reproduction_rate) %>% 
    replace(is.na(.), 0) %>% 
    range()
```
...
Thus, want to scale: `icu_patients, hosp_patients, new_test, new_vaccinations`

```{r}

fmxdat::source_all("./code")

world_df <- world_df %>% scale_bigs(world_df)
...
world_df %>% ungroup() %>% group_by(location) %>% 
    ...
    ggplot() +
    geom_point(aes(x = location, y = cardiovasc_death_rate)) +
    theme(axis.text.x = element_blank(),
      axis.title = element_text(face = "bold"),
      axis.line = element_line(colour = "grey50", size = 1)) +
    scale_y_continuous("Cardiovascular Death Rate") +
    labs(title = "Plot of Distribution of Cardiovascular Death Rate per Country")
```
...
world <- world_df %>% scale_bigs_constant()
...
```{r}
options(scipen=999)
world_df %>% ungroup() %>% select(-c(location, date)) %>% describe()
options(scipen=0)
```
